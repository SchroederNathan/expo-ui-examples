...
/** Material 3 `MenuDefaults.shape` is the extra-small shape: 4dp corners. */
const MENU_RADIUS = 4;

/**
 * The ⋮ button is BUTTON_SIZE wide and sits at the card's trailing edge, which is
 * BUTTON_OVERHANG past the photo. Material's `more_vert` ink spans 10–14dp of its
 * 24dp box, so the dots end 2dp right of the button's center; with these numbers
 * that edge lands exactly on the photo's edge (16 − 18 + 2 = 0). Keeping the whole
 * button inside the card is what lets its ripple draw without being clipped.
 */
export const BUTTON_OVERHANG = 16;
...
export function PhotoCard({
  ...
  return (
    <Column modifiers={[widthModifier(width + BUTTON_OVERHANG)]} verticalArrangement={{ spacedBy: 4 }}>
      {/* 4dp — the same extra-small shape Material gives the DropdownMenu
          (`MenuDefaults.shape`), so the photo and its menu share one radius. */}
      <Box modifiers={[size(width, width), clip(Shapes.RoundedCorner(MENU_RADIUS))]}>
      ...
      <Row modifiers={[fillMaxWidth()]} verticalAlignment="center">
        ...
        <DropdownMenu expanded={open} onDismissRequest={close}>
          <DropdownMenu.Trigger>
            {/* See BUTTON_OVERHANG: the dots end on the photo's edge and the ripple
                stays whole. Material still gives the button a 48dp touch target. */}
            <IconButton onClick={onOpen} modifiers={[size(BUTTON_SIZE, BUTTON_SIZE)]}>
