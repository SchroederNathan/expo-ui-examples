...
- Keep the example **self-contained in its folder** so it can be dropped into any SDK 58 project as-is.
- **Icons are never generated by hand.** iOS uses SF Symbols by name (`systemImage`, `Image systemName`). Android imports Material Symbols from `@expo/material-symbols`, one subpath per icon — `import Check from '@expo/material-symbols/check.xml'` — for `Icon source`, `Icon.select({ android })`, and the registry `materialIcon`. Only the outlined style ships.
- Confirm APIs against the installed types — `node_modules/@expo/ui/build/<swift-ui|jetpack-compose|universal>/<Component>/index.d.ts` — and the versioned docs (https://docs.expo.dev/versions/v58.0.0/), per AGENTS.md.
...
- **It may own route files under `src/app/<slug>/`.** A zoom transition needs a real push onto
  ...
  folder, so the example is still one self-contained directory. If the screen renders
  platform-only views (`@expo/ui/swift-ui` or `jetpack-compose`), redirect to `/` on the other
  platform instead, as `src/app/mini-player/` does — deep links can reach any route file.
  `[slug].tsx` already does this for registry examples. A folder named after the slug
...
Then check it in the simulator with argent MCP tools. Expo Go can't run this app — `@expo/ui` needs native code, so it runs as a dev client (`com.exponathan.expouiexamples`):
...
2. If the dev client isn't installed on the simulator yet, build it: `bunx expo run:ios --port 8090 --device <UDID>` (Android: `bunx expo run:android --port 8090` with the emulator booted; `--device` takes the AVD name). Otherwise `launch-app` with the bundle id above.
3. The first launch shows an "Open in expo-ui-examples?" prompt and then the dev menu onboarding sheet — dismiss both.
4. `await-ui-element` for the new example's title, `describe` to get the row's coordinates, `gesture-tap` it, and confirm the screen renders with the right nav title.
...
- Fast Refresh covers JS edits; `restart-app` gives a full reload. Adding or removing a native dependency needs a new dev client build.

Universal-layer gotchas (first verified against `@expo/ui` 57; the repo now runs 58, so re-check the platform source if one no longer holds):
...
- **A universal `onPress` doesn't hit-test empty padding on iOS.** It becomes `onTapGesture` after the padding, with no `contentShape`, so only drawn pixels are tappable. Give a padded touch target a fill (`rgba(0, 0, 0, 0.001)` is invisible) — see the back button in `universal-settings.tsx`. Compose's `clickable` covers the padding already.
- **Accessibility props are thin in the universal layer.** `Icon`'s `accessibilityLabel` reaches TalkBack but is not wired up on iOS yet, and `Row` has none.
